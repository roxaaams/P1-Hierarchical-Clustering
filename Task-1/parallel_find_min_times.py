# -*- coding: utf-8 -*-
"""
Created on Sun Apr 18 16:17:30 2021
...
jobs = -1
for i in range(200,8001,200):
    test_data = np.random.rand(i,2)
    distance_matrix = pair_dist(test_data)
    start_linear = time.time()
    find_min_single(distance_matrix, jobs)
    stop_linear = time.time()
    start_parallel = time.time()
    find_min_wrapper(distance_matrix, jobs)
    stop_parallel = time.time()
    print("size: ", i)
    print("singular: ", stop_linear -start_linear)
